...
function ListDetailsPage(props) {
    ...
    const { copiedItems, addToCopiedItems, removeFromCopiedItems } = useContext(CartContext);
    ...
    const AddAlert = (status, data) => {
        addAlert({ id: uuidv4(), msg: data.message, ok: status });
    };

    const updateCartHandler = async () => {
        const cart = products.map((product) => ({ productId: product._id, amount: product.amount }));
    ...
    const addProductToListHandler = (product) => {
        ...
        const searchedProduct = products.find((prod) => prod._id === product._id);
                    ...
                    <AnimatePresence>
                        {
                            products.map((product, index) => (
                                <ListItem
                                    key={product._id}
                                    index={index}
                                    check={addToCopiedItems}
                                    uncheck={removeFromCopiedItems}
                                    checked={copiedItems.indexOf(product._id) > -1}
                                    {...product}
                                />
                            ))
                        }
